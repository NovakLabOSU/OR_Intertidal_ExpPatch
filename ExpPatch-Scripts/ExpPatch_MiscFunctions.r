##############################################################
# Miscellaneous functions that are useful in multiple contexts
##############################################################
...
ptscale <- function(x, Scale = 1, base = 4) {
  return(Scale * log(x, exp(base)))
}
...
var.wtd.mean.cochran <- function(x, w, na.rm = TRUE) {
  ...
  wbar = mean(w)
  out = n / ((n - 1) * sum(w) ^ 2) * (sum((w * x - wbar * xWbar) ^ 2) - 2 *
                                        xWbar * sum((w - wbar) * (w * x - wbar * xWbar)) + xWbar ^ 2 * sum((w -
                                                                                                              wbar) ^ 2))
  return(out)
}